...
function fmtDiff(diffM, isToday, depDate, nowDate){
  // diffM: 분 차이
  // isToday: 오늘 여부
  if(!isToday){
    const totalMins = diffM;
    // 날짜 차이 계산
    const d1 = new Date(depDate); d1.setHours(0,0,0,0);
    const d2 = new Date(nowDate); d2.setHours(0,0,0,0);
    const dayDiff = Math.round((d1-d2)/(1000*60*60*24));
    const hh = Math.floor(totalMins/60), mm = totalMins%60;
    if(dayDiff>0){
      if(hh>0&&mm>0) return `${dayDiff}일 ${hh}시간 ${mm}분 후`;
      if(hh>0) return `${dayDiff}일 ${hh}시간 후`;
      return `${dayDiff}일 ${mm}분 후`;
    }
  }
  const hh=Math.floor(diffM/60), mm=diffM%60;
  if(hh>0&&mm>0) return `${hh}시간 ${mm}분 후`;
  if(hh>0) return `${hh}시간 후`;
  return `${mm}분 후`;
}
...
function SecHead({title,count,titleSize=14}) {
  return <div style={{padding:'9px 14px',background:C.surf,borderBottom:`1px solid ${C.border}`,
    display:'flex',justifyContent:'space-between',alignItems:'center'}}>
    <span style={{fontWeight:800,fontSize:titleSize}}>{title}</span>
    ...
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginBottom:6}}>
      <div style={{position:'relative'}}>
        <select value={stn} onChange={e=>onChange(e.target.value)} style={{
          background:C.surf,color:C.accent,border:`1.5px solid ${C.accent}`,
          borderRadius:9,padding:'6px 28px 6px 12px',
        ...
        <span style={{position:'absolute',right:8,top:'50%',transform:'translateY(-50%)',
          pointerEvents:'none',color:C.accent,fontSize:13,fontWeight:700}}>∨</span>
      </div>
    </div>
  );
}
...
// ── TrainList: 다음열차 목록 ──
// effectiveNowM: 슬라이스 기준 (어떤 열차부터 보여줄지)
// realNowM: 실제 현재시각 (배지 표시 기준)
// isToday: 오늘 여부 (다른 날이면 배지 숨김)
function TrainList({slice,accentCol,nowFirstId,effectiveNowM,realNowM,isToday}){
  if(slice.length===0)return(
    <div style={{padding:12,textAlign:'center',color:C.muted,fontSize:12}}>운행 종료</div>
  );
  return slice.map((t)=>{
    const stM=t.stM;
    const isNow=t.id===nowFirstId;
    const loColor=accentCol===C.accent?C.lo:'#3B82F618';
    // 배지 표시 조건: 오늘이고, 실제 현재시각 이후 열차만
    const showBadge = isToday && stM >= realNowM;
    return <div key={t.id} style={{
      ...
      {stM<effectiveNowM
        ? <span style={{fontSize:10,color:C.muted}}>통과</span>
        : showBadge
          ? <Badge stM={stM} nowM={realNowM}/>
          : null
      }
    </div>;
  });
  ...
  return <>
    ...
    <div style={{...scrollS,paddingTop:8,paddingBottom:60}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8,flexWrap:'wrap',justifyContent:'center'}}>
        ...
        <NowBtn onReset={()=>{onDateChange(new Date());setSelHour(-1);}}/>
        <HolBadge isHol={dateHol}/>
      </div>

      {/* 환승 정보 + BY LEE YANG-HO */}
      {xfer&&<div style={{display:'flex',gap:6,marginBottom:4,flexWrap:'wrap'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
        ...
        <div style={{...cardS,marginBottom:0}}>
          <div style={{padding:'7px 8px',background:C.surf,borderBottom:`1px solid ${C.border}`,
            textAlign:'center',fontSize:13,fontWeight:700,color:C.accent}}>← 구미행</div>
          {upAll.length===0
            ? <div style={{padding:10,textAlign:'center',color:C.muted,fontSize:12}}>운행 종료<br/><span style={{fontSize:11}}>첫차 {fmtM(upFirst)}</span></div>
            : <>
              <TrainList slice={upSlice} accentCol={C.accent} nowFirstId={upAll[upNowIdx]?.id} effectiveNowM={effectiveNowM} realNowM={nowM} isToday={isToday}/>
        ...
        <div style={{...cardS,marginBottom:0}}>
          <div style={{padding:'7px 8px',background:C.surf,borderBottom:`1px solid ${C.border}`,
            textAlign:'center',fontSize:13,fontWeight:700,color:C.blue}}>경산행 →</div>
          {dnAll.length===0
            ? <div style={{padding:10,textAlign:'center',color:C.muted,fontSize:12}}>운행 종료<br/><span style={{fontSize:11}}>첫차 {fmtM(dnFirst)}</span></div>
            : <>
              <TrainList slice={dnSlice} accentCol={C.blue} nowFirstId={dnAll[dnNowIdx]?.id} effectiveNowM={effectiveNowM} realNowM={nowM} isToday={isToday}/>
...
function RouteTab({nowM, hol, selectedStn, onStnChange, selectedDate, onDateChange}) {
  ...
  const [selMode,setSelMode]=useState('from'); // 'from'|'to' — 다음 터치가 출발/도착
  ...
  // 노선도 역 터치 → 첫 터치=출발, 두번째=도착, 이후 반복
  function handleMapStn(n){
    if(selMode==='from'){
      changeFrom(n);
      setSelMode('to');
    } else {
  ...
  return <div style={scrollS}>
    ...
    <div style={cardS}>
      <div style={{padding:'10px 12px'}}>
        <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:10,justifyContent:'center',flexWrap:'wrap'}}>
          ...
          <NowBtn onReset={()=>{onDateChange(new Date());setSelHour(-1);setSearched(false);}}/>
          <HolBadge isHol={dateHol}/>
        </div>
        ...
        <div style={{display:'flex',gap:6,alignItems:'flex-end',marginBottom:10}}>
          <div style={{flex:1}}>
            <button onClick={()=>setSelMode('from')} style={{
              width:'100%',background:selMode==='from'?C.accent+'33':C.surf,
              border:`2px solid ${selMode==='from'?C.accent:C.border}`,
              borderRadius:9,padding:'4px 0 2px',cursor:'pointer',fontFamily:'inherit',
            }}>
              <div style={{fontSize:11,color:selMode==='from'?C.accent:C.muted,fontWeight:700,textAlign:'center'}}>
                {selMode==='from'?'▼ 출발 선택중':'출발'}
              </div>
              <select style={{...selS,textAlign:'center',fontSize:14,fontWeight:700,
                background:'transparent',border:'none',padding:'4px 8px',width:'100%'}}
          ...
          <button onClick={swap} style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:9,
            padding:'8px 10px',cursor:'pointer',color:C.accent,fontSize:20,fontFamily:'inherit',
            fontWeight:700,flexShrink:0,marginBottom:2}}>⇄</button>
          <div style={{flex:1}}>
            <button onClick={()=>setSelMode('to')} style={{
              width:'100%',background:selMode==='to'?C.blue+'33':C.surf,
              border:`2px solid ${selMode==='to'?C.blue:C.border}`,
              borderRadius:9,padding:'4px 0 2px',cursor:'pointer',fontFamily:'inherit',
            }}>
              <div style={{fontSize:11,color:selMode==='to'?C.blue:C.muted,fontWeight:700,textAlign:'center'}}>
                {selMode==='to'?'▼ 도착 선택중':'도착'}
              </div>
              <select style={{...selS,textAlign:'center',fontSize:14,fontWeight:700,
                background:'transparent',border:'none',padding:'4px 8px',width:'100%'}}
        ...
        <div style={{background:C.bg,borderRadius:10,border:`1px solid ${C.border}`,
          padding:'8px 4px 6px',marginBottom:10,overflowX:'auto'}}>
          {/* 안내 텍스트 */}
          <div style={{textAlign:'center',fontSize:10,color:C.muted,marginBottom:6}}>
            {selMode==='from'?'🟠 출발역을 터치하세요':'🔵 도착역을 터치하세요'}
          </div>
          <div style={{display:'flex',alignItems:'center',minWidth:320,position:'relative',
            paddingTop:32,paddingBottom:32}}>
            <div style={{position:'absolute',top:'50%',left:12,right:12,height:4,
              background:`linear-gradient(to right,${C.accent},${C.blue})`,
              borderRadius:2,transform:'translateY(-50%)',zIndex:0}}/>
            {ROUTE_STNS.map((s,i)=>{
              const isSel = s.n===from || s.n===to;
              const isFrom = s.n===from;
              const isTo   = s.n===to;
              const hasXfer= !!s.xfer;
              const labelUp= i%2===0;
              const dotCol = isFrom?C.accent:isTo?C.blue:hasXfer?'#FBBF24':C.surf;
              const txtCol = isFrom?C.accent:isTo?C.blue:hasXfer?'#FBBF24':C.text;
              const labelSz= isSel?13:10;
              return (
                <div key={s.n} onClick={()=>handleMapStn(s.n)}
                  style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',
                    position:'relative',zIndex:2,cursor:'pointer'}}>
                  {labelUp
                    ? <div style={{height:24,display:'flex',alignItems:'flex-end',marginBottom:3}}>
                        <span style={{fontSize:labelSz,fontWeight:isSel?800:500,color:txtCol,
                          textAlign:'center',whiteSpace:'nowrap',
                          animation:isSel?'pulse 1.2s infinite':'none'}}>
                          {isFrom?'출발':isTo?'도착':''}{s.n}
                        </span>
                      </div>
                    : <div style={{height:27}}/>
                  }
                  <div style={{
                    width:isSel?18:hasXfer?13:9,height:isSel?18:hasXfer?13:9,
                    borderRadius:'50%',
                    background:dotCol,
                    border:`2px solid ${isSel?dotCol:hasXfer?'#FBBF24':C.muted}`,
                    boxShadow:isSel?`0 0 12px ${dotCol}AA`:hasXfer?'0 0 6px #FBBF2466':'none',
      ...
      <div style={cardS}>
        ...
        {res.length===0
          ...
          : res.map((r,i)=>{
            ...
            const hh=Math.floor(travel/60), mm=travel%60;
            const travelLabel=hh>0?`${hh}시간 ${mm}분`:`${mm}분`;
            return (
            <div key={r.id} style={{padding:'10px 14px',borderBottom:`1px solid ${C.border}`,
              background:i===0&&isToday?C.lo:'transparent'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
                ...
                {/* 오늘 + 실제 현재시각 이후 열차만 배지 표시 */}
                {isToday&&r.depM>=nowM&&<Badge stM={r.depM} nowM={nowM}/>}
              </div>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <span style={{background:'#1A3A6E',color:'#7BAFD4',
                  border:'1px solid #2A5090',borderRadius:6,padding:'2px 8px',
                  fontSize:12,fontWeight:700}}>소요 {travelLabel}</span>
                <span style={{fontSize:12,color:C.muted,fontWeight:600}}>{r.id}</span>
              </div>
            </div>
          );})
        }
  ...
  return <div style={scrollS}>
    ...
    <div style={cardS}>
      <div style={{padding:'9px 14px',background:C.surf,borderBottom:`1px solid ${C.border}`,
        ...
        <span style={{fontWeight:800,fontSize:20,letterSpacing:-0.3}}>역명 터치 선택</span>
      </div>
    ...
    <div style={cardS}>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',
        ...
        <div style={{padding:'7px 4px',textAlign:'center',
          ...
          fontSize:22,fontWeight:700,color:C.accent}}>← 구미행</div>
        <div style={{padding:'7px 4px',textAlign:'center',
          fontSize:22,fontWeight:700,color:C.blue}}>경산행 →</div>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr'}}>
        {/* 구미행 — 가운데 정렬 */}
        <div style={{borderRight:`1px solid ${C.border}`}}>
          {mapUp.length===0
            ...
            : mapUp.map((t,i)=>{
              ...
              return <div key={t.id} style={{padding:'7px 4px',borderBottom:`1px solid ${C.border}`,
                background:i===0&&upOff===0?'#FF7A0008':'transparent',textAlign:'center'}}>
        ...
        <div>
          {mapDn.length===0
            ...
            : mapDn.map((t,i)=>{
              ...
              if(stM===null)return null;
              return <div key={t.id} style={{padding:'7px 4px',borderBottom:`1px solid ${C.border}`,
                background:i===0&&dnOff===0?'#3B82F608':'transparent',textAlign:'center'}}>
                <div style={{fontFamily:"'Space Mono',monospace",fontSize:22,fontWeight:700,
...
// ── ColRows: 시간표 한 열 렌더러 ──
function ColRows({rows, ni, accentCol, colRef, pivot, isToday, realNowM}){
  return rows.map((r,i)=>{
    const past = pivot >= 0 && r.stM < pivot;
    const isNext = i === ni && pivot >= 0;
    // 분후 표시 조건: 오늘이고, 실제 현재시각 이후 열차만
    const showMin = isToday && realNowM !== undefined && r.stM >= realNowM;
    return <div key={r.id} ref={isNext?colRef:null} style={{
      ...
      {isNext&&showMin&&<span style={{fontSize:9,color:accentCol,fontWeight:700,marginTop:1,
        animation:'pulse 1.3s infinite'}}>
        {r.stM-realNowM<=0?'출발중':r.stM-realNowM<=2?'곧출발':`${r.stM-realNowM}분후`}
      </span>}
...
function TTTab({nowM, hol, selectedStn, onStnChange, selectedDate, onDateChange}) {
  ...
  const upColRef=useRef(null);
  const dnColRef=useRef(null);
  ...
  // 각 열 독립적으로 스크롤: 해당 열 컨테이너 기준으로 scrollTop 조정
  useEffect(()=>{
    const timer=setTimeout(()=>{
      // 구미행 열 스크롤
      if(upRef.current && upColRef.current){
        const rowH = upRef.current.offsetHeight||44;
        const targetIdx = upNi >= 0 ? upNi : 0;
        const scrollTo = Math.max(0, targetIdx * rowH - upColRef.current.offsetHeight/2);
        upColRef.current.scrollTop = scrollTo;
      }
      // 경산행 열 스크롤
      if(dnRef.current && dnColRef.current){
        const rowH = dnRef.current.offsetHeight||44;
        const targetIdx = dnNi >= 0 ? dnNi : 0;
        const scrollTo = Math.max(0, targetIdx * rowH - dnColRef.current.offsetHeight/2);
        dnColRef.current.scrollTop = scrollTo;
      }
    },80);
    return ()=>clearTimeout(timer);
  },[stn,dateHol,selHour,nowM]);

  // 화면 높이 기반으로 열 높이 계산 (헤더 약 130px, 탭바 60px 제외)
  const colHeight = 'calc(100dvh - 200px)';

  return <div style={{flex:1,padding:'8px 10px',paddingBottom:0,display:'flex',flexDirection:'column',overflow:'hidden'}}>
    {/* sticky 헤더 */}
    <div style={{
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',flex:1,
      ...
      borderRadius:'0 0 10px 10px',overflow:'hidden',marginBottom:80}}>
      {/* 구미행 열 */}
      <div ref={upColRef} style={{overflowY:'auto',borderRight:`1px solid ${C.border}`}}>
        <ColRows rows={upRows} ni={upNi} accentCol={C.accent} colRef={upRef} pivot={pivotM} isToday={isToday} realNowM={nowM}/>
      </div>
      {/* 경산행 열 */}
      <div ref={dnColRef} style={{overflowY:'auto'}}>
        <ColRows rows={dnRows} ni={dnNi} accentCol={C.blue} colRef={dnRef} pivot={pivotM} isToday={isToday} realNowM={nowM}/>
      </div>
    </div>
  </div>;
}

// ── NowBtn: 오늘/현재 리셋 버튼 — DatePicker와 동일한 디자인 ──
function NowBtn({onReset}){
  return(
    <button onClick={onReset} style={{
      background:C.surf,
      border:`1px solid ${C.border}`,
      borderRadius:9,padding:'6px 10px',cursor:'pointer',
      fontFamily:'inherit',fontSize:13,fontWeight:700,color:C.text,
...
// 탭 순서: 다음열차(0), 경로검색(노선도)(1), 시간표(2)
const TABS=[
  {icon:null,label:'다음열차',svgIcon:true},
  {icon:'🔍🗺️',label:'경로검색(노선도)'},
  {icon:'📋',label:'시간표'},
];
      ...
      <div style={{background:C.surf,borderBottom:`1px solid ${C.border}`,padding:'10px 16px 9px',flexShrink:0}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <div style={{display:'flex',alignItems:'center',gap:9}}>
            ...
            <div>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <div style={{fontWeight:800,fontSize:20,letterSpacing:-0.3}}>대경선</div>
